...
for sample_size in [10]:#, 50, 100, 500, 1000, 5000]:
    for epsilon in epsilons:
        ...
        for fact, cfs in zip(input_set, cf_set):
            diffs = fact - cfs
            diffs[:, discrete_mask] = diffs[:, discrete_mask] != 0 # one for every changed feature.
        ...
        for i, context in enumerate(encoder.context):
            ...
            for output_class in classes:
                ...
                wasserstein = compute_wd(np.concatenate(cf_set[~mask])[:, i], input_set[mask, i], is_categorical=(context.scale==0), bin_edges=None)
                data.append({
