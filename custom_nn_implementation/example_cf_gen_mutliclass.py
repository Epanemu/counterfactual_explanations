...
# train the NN
model = NNModel(encoded.shape[1], hidden_sizes=[15, 30], output_size=10)
to_train = False
if to_train:
...
i = 0
in_data = X_test[i]
# print("Prediction vals:", model.predict(encoder.encode_datapoint(in_data)))
prediction = np.argmax(model.predict(encoder.encode_datapoint(in_data)).numpy())
