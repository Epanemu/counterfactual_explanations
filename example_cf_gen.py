...
# train the NN
model_nn = NNModel(encoded.shape[1], hidden_sizes=[15, 10], output_size=1)
to_train = False
if to_train:
    model_nn.train(X_train, y_train, batch_size=128, epochs=50)
    model_nn.save(model_path)
else:
    model_nn.load(model_path)
...
print("Prediction:", int(model_nn.predict(encoder.encode_datapoint(in_data)) >= 0))
...
counterfactuals = cf_generator.generate_n_counterfactuals(in_data, model_nn, n, cf_margin=margin)
for cf in textualizer.formulate_list(counterfactuals, labels=("BAD", "GOOD")):
    print(cf)
...
if get_by_distance:
    ...
    counterfactuals = cf_generator.generate_close_counterfactuals(in_data, model_nn, relative_distance_q, n_limit=100, verbose=False)
    for cf in textualizer.formulate_list(counterfactuals, labels=("BAD", "GOOD")):
        print(cf)
...
if explain_all:
    ...
    counterfactuals_list = cf_generator.explain_set(input_data.values, model_nn, epsilon=1)
    textual_data = []
...
if custom_change:
    ...
    print("Prediction:", model_nn.predict(encoder.encode_datapoint(in_data)))
    n = 1
    print(f"{n} best counterfactuals:")
    counterfactuals = cf_generator.generate_n_counterfactuals(in_data, model_nn, n)
    for cf in textualizer.formulate_list(counterfactuals, labels=("BAD", "GOOD")):
